 ...
 */
public class PurchaseAlertsApp {
    ...
    private static final long MAX_INCOME_PER_MINUTE = 3000L;
    ...
    public static Topology buildTopology(SchemaRegistryClient client, Map<String, String> serDeConfig) {
        ...
        purchaseWithJoinedProduct.groupBy((key, val) -> val.get("productid").toString(), Grouped.with(new Serdes.StringSerde(), avroSerde))
                ...
                .aggregate(
                        () -> 0L,
                        (key, val, agg) -> agg += (Long) val.get("purchase_quantity") * (Double) val.get("purchase_price"),
                        Materialized.with(new Serdes.StringSerde(), new Serdes.LongSerde())
                )
                .filter((key, val) -> val > MAX_INCOME_PER_MINUTE)
                .toStream()
                .map((key, val) -> {
                    // создаем схему нашего алерта
                    Schema schema = SchemaBuilder.record("IncomeAlert").fields()
                            .name("window_start")
